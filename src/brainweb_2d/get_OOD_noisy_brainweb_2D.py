...
if __name__ == "__main__":
    ...
    datasets = ["test","test_tumour"]
    xp.random.seed(0)
    file_directory = "path_to/"
    for name in datasets:

        path_to_files=  name+"_clean.pt"
        dataset = BrainWebClean(path_to_files=path_to_files)
        trues_per_volumes = [5, 7.5, 100]
        for trues_per_volume in trues_per_volumes:
            ...
            print(f"Dataset {name} trues per volume {trues_per_volume}")
            for idx in tqdm(range(len(dataset))):
                y, mu, gt = dataset[idx]
                gt = xp.asarray(gt.squeeze().unsqueeze(-1).numpy())
                mu = xp.asarray(mu.squeeze().unsqueeze(-1).numpy())
                y = xp.asarray(y.numpy().squeeze())
                ...
                osem_pts.append(torch.from_dlpack(osem_x[:,:,0])[None,None].float().cuda())
                scaling_factor_pts.append(torch.tensor(scaling_factor)[None,None].float().cuda())
                noisy_data_pts.append(torch.from_dlpack(data)[None,None].float().cuda())
                contamination_pts.append(torch.tensor(contamination_scale)[None,None].float().cuda())
                attenuation_pts.append(torch.from_dlpack(attenuation_factors)[None,None].float().cuda())
            ...
            torch.save(save_dict, file_directory + f"/noisy/{name}_noisy_{trues_per_volume}.pt")
