...
if __name__=="__main__":
    ...
    tumour = True
    if tumour:
        print('Getting true data with tumours...')
    clean_data_pts = []
    mu_ref_pts = []
    image_ref_pts = []
    mri_ref_pts = []
    ...
    for subject_number in tqdm(subjects):
        for sim_number in range(3):
            nii_pet = nib.as_closest_canonical(nib.load(f'path_to/examples/data/brainweb_petmr/subject{subject_number:02}/sim_{sim_number}/true_pet.nii.gz'))
            nii_mu = nib.as_closest_canonical(nib.load(f'path_to/examples/data/brainweb_petmr/subject{subject_number:02}/mu.nii.gz'))
            nii_mri = nib.as_closest_canonical(nib.load(f'path_to/examples/data/brainweb_petmr/subject{subject_number:02}/t1.nii.gz'))
    ...
    if tumour:
        ...
        torch.save(recon_dict, "path_to/clean/clean_train_tumour.pt")
    else:
        recon_dict = {'clean_measurements': clean_data_pts, 'mu': mu_ref_pts, 'reference': image_ref_pts, 'mri': mri_ref_pts}
        torch.save(recon_dict, "path_to/clean/clean_train.pt")
